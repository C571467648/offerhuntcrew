...
- **双平台兼容**：同时支持 **TraeWork**（推荐，功能更强、免费额度更多、腾讯文档 MCP 一键添加）和 **Trae**（基础版，也完全可用），总架构师自动适配。
- **可定制**：懂 AI / Agent 的同学可以让总架构师调整架构，适配自己的情况。

> 💡 **推荐用 TraeWork**：功能更强（腾讯文档 MCP 直接读校招汇总表、更多内置 Skill、插件系统），免费额度也更多。Trae 也完全能用，但 TraeWork 体验更好。
...
| 材料 | 说明 |
|---|---|
| AI 客户端 | **推荐 TraeWork**（功能更强、免费额度更多、腾讯文档 MCP 一键添加）。**Trae** 也完全可用（基础版）。Cursor 等其他支持 MCP 的客户端也可以 |
| 大模型 | **先用客户端自带免费额度**；不够再配 DeepSeek/Qwen 等（见下方推荐，具体配置问总架构师） |
| 个人材料 | 简历（PDF/Word）、成绩单（可选）、求职目标 |

> 📌 **TraeWork vs Trae 怎么选？**
> - **首推 TraeWork**：功能更强、免费额度更多，腾讯文档 MCP 可直接读取校招汇总表（不用浏览器模拟），还有浏览器插件、知识图谱记忆、xlsx/docx/pdf 等内置 Skill，体验提升明显。
> - **Trae 也能用**：基础功能完整，适合习惯用 Trae 的用户。
> - 不管选哪个，**总架构师都会自动适配**——它会检测你用的是什么客户端，然后用最合适的方案。
...
| 模型 | 说明 |
|---|---|
| **客户端免费额度** | TraeWork / Trae 均自带免费额度。建议**先用它把整个系统跑通**，一分钱不花 |
| DeepSeek V4 Flash | 额度用完后可配。⚠️但是涨价后较贵，建议在**非高峰期**使用 |
| Qwen 3.7 Flash / 3.8 Flash | DeepSeek 涨价时的备选，同样便宜好用 |
| 其他模型 | 你也可以用任何你喜欢的模型，具体选哪个由你自己决定 |

> - **先用免费的**：客户端自带额度足以支撑日常使用，先用着。
> - **额度用完了**：再配置你自己的大模型 API。**怎么配、配哪个、要不要配——直接问你的总架构师**，它会一步步带你完成，你完全不用自己研究。
> - 本系统不绑定任何特定模型或客户端，配置方式可随时调整。
...
**Q：TraeWork 和 Trae 有什么区别？我该选哪个？**
A：**推荐 TraeWork**——功能更强、免费额度更多，腾讯文档 MCP 可直接读取校招汇总表（不用浏览器模拟），选岗效率提升明显。Trae 也完全能用，但 TraeWork 体验更好。具体差异见 `02_系统配置/平台适配/平台对比.md`。

**Q：TraeWork 的腾讯文档 MCP 有什么用？**
A：选岗老师可以直接通过 API 读取腾讯文档的校招汇总表（~6000 条秋招数据），不用模拟浏览器打开页面，速度快、数据全、稳定性高。这是 TraeWork 用户的专属效率提升。
